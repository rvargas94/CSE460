...
void VirtualMachine::run(int time_slice, fstream & in, fstream & out)
{
    ...
    while (pc < base+limit && clock < interrupt) {
        ...
        if (opcode == 0) { /* load loadi */
            ...
            // set OVERFLOW
            if (sign1 == sign2 && sign1 != (r[rd]&0x8000)>>15)
                sr |= 0x10;
            else
                sr &= 0xf;
            // sign extend
            ...
            // set OVERFLOW
            if (sign1 != sign2 && sign2 == (r[rd]&0x8000)>>15)
                sr |= 0x10;
            else
                sr &= 0xf;
            // sign extend
            ...
            sr &= 0xffffff1f; sr |= 0x40; // HALT
            return;
        } else if (opcode == 25) { /* noop */
            /* do nothing */
        } else {
            cerr << "Bad opcode = " << opcode << endl;
            sr &= 0xffffff1f; sr |= 0xc0; // BAD OP
            return;
        }
        if (debug) {
